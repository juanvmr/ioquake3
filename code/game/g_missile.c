...
/*void Guided_Plasma_Think (gentity_t *plasma) {
    vec3_t forward, right, up;
    vec3_t muzzle;
    float dist;
    gentity_t *target = plasma->target_ent;
    ...
    AngleVectors (target->client->ps.viewangles, forward, right, up);
    // Calculate the player's eyes origin, and store this origin in muzzle
    CalcMuzzlePoint ( target, forward, right, up, muzzle );
    ...
    plasma->s.pos.trType = TR_LINEAR;
    plasma->s.pos.trTime = level.time - 50;
    ...
    vectoangles (muzzle, plasma->s.angles);
...
gentity_t *findNearestTargetInRadius (gentity_t *ent, float radius, vec3_t *pTargetDirectionVector,
                                      vec3_t missileNormalizedForwardDirection, double fVisionCone)
{
    ...
    for (pEntity = g_entities; pEntity < &g_entities[level.num_entities]; pEntity++)      {
            ...
            if (fEntityMidBodyVectorLengh > radius){
                continue;
            }
            ...
            if(pEntity!=NULL)
            {
                ...
                if (DotProduct(missileNormalizedForwardDirection, temp_vector) < fVisionCone ){
                    //ent->think = G_ExplodeMissile;
                    continue;  // Check if visible
                }

                trap_Trace (&trace, ent->s.pos.trBase, NULL, NULL, pEntity->s.pos.trBase, ent->s.number, MASK_SHOT );
...
void Plasma_Think( gentity_t *ent ) {
    ...
    VectorCopy(ent->s.pos.trDelta, forward);
    VectorNormalize(forward);
    pTarget = findNearestTargetInRadius(ent, ent->splashRadius*5, &targetdir, forward,0.5);

    if(pTarget!=NULL)
    {
        //ent->think = pTarget->think;
        // Steer the missile to the target.
        // Lower the value in place of 0.05, larger the turning circle.
        VectorMA(forward, 0.1, targetdir, targetdir);
        VectorNormalize(targetdir);
        VectorScale(targetdir, ent->speed, ent->s.pos.trDelta);
    }
...
void Guided_Missile_Think (gentity_t *missile) {
    ...
    // have come back if player was aiming on a block while the rocket is behind it
    // as the trace stops on solid blocks
    dist = VectorLength (muzzle) + 100; //give the range of our muzzle vector + 400 units
    VectorScale (forward, dist, forward);
    // line straight forward
    VectorAdd (forward, muzzle, muzzle);
    // Normalize the vector so it's 1 unit long, but keep its direction
